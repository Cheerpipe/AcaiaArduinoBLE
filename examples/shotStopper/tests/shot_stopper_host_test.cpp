...
void w12_hard_limit_cannot_be_configured_above_fifty_seconds() {
  RuntimeConfig config;
...
void w13_virtual_paddle_uses_normal_state_machine() {
  ...
  reachReadyFromBoot();
  WebCommand on;
  on.type = WebCommandType::PADDLE_ON;
  ...
  CHECK(stopperState == StopperState::MANUAL_NO_SCALE);
  WebCommand off;
  off.type = WebCommandType::PADDLE_OFF;
...
void w29_operational_wall_of_50001_is_rejected() {
  RuntimeConfig config;
  config.operationalWallMs = 50001;
  CHECK(validateRuntimeConfig(config) ==
        ConfigValidationError::OPERATIONAL_WALL);
}
...
const TestCase testCases[] = {
    ...
    {"W12", w12_hard_limit_cannot_be_configured_above_fifty_seconds},
    ...
    {"W29", w29_operational_wall_of_50001_is_rejected},
