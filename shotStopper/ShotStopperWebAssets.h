...
<main id="app">
  ...
  <fieldset><legend>Shot history</legend>
    ...
    <small>Up to 120 shots. Clock sync uses NTP when Wi‑Fi STA has internet; otherwise time falls back to uptime since boot.</small>
  </fieldset>
...
function formatShotTime(r,tz){if(r.endedAtUnixSec)return formatWallTime(r.endedAtUnixSec,tz);let adjusted=Math.floor(r.endedAtMs/1000)+tz*60;const day=Math.floor(adjusted/86400);adjusted=((adjusted%86400)+86400)%86400;const time=pad2(Math.floor(adjusted/3600))+':'+pad2(Math.floor((adjusted%3600)/60))+':'+pad2(adjusted%60);return day>0?'#'+r.bootId+' d'+day+' '+time+' ~':'#'+r.bootId+' '+time+' ~'}
function formatShotTimeCsv(r,tz){if(r.endedAtUnixSec)return formatWallTime(r.endedAtUnixSec,tz);let adjusted=Math.floor(r.endedAtMs/1000)+tz*60;const day=Math.floor(adjusted/86400);adjusted=((adjusted%86400)+86400)%86400;const time=pad2(Math.floor(adjusted/3600))+':'+pad2(Math.floor((adjusted%3600)/60))+':'+pad2(adjusted%60);return day>0?'d'+day+' '+time:time}
