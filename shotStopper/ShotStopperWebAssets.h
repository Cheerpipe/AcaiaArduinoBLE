...
<main id="app">
  ...
  <fieldset id="workflowPanel"><legend>Workflow</legend>
    ...
    <div class="row">
      ...
      <label>Rinse gesture (s)<input id="rinseGestureS" type="number" min="0.1" max="5" step="0.1" required></label>
      <label>Rinse duration (s)<input id="rinseDurationS" type="number" min="0.5" max="10" step="0.1" required></label>
      <label>Brew confirmation (s)<input id="brewConfirmS" type="number" min="0.5" max="10" step="0.1" required></label>
      <label>Minimum auto-stop (s)<input id="minAutoStopS" type="number" min="1" max="30" step="1" required></label>
      <label>CN9 limit (s)<input id="operationalWallS" type="number" min="5" max="60" step="1" required></label>
    </div>
    ...
    <label>Paddle reminder interval (s)<input id="paddleReturnReminderIntervalS" type="number" min="5" max="60" step="1" required></label>
    <label>Paddle reminder limit (s)<input id="paddleReturnReminderMaxDurationS" type="number" min="60" max="3600" step="1" required></label>
    ...
    <small>Required: rinse gesture &lt; brew confirmation &lt; minimum auto-stop &lt; CN9 limit ≤ 60 s.</small>
  </fieldset>
  ...
  <fieldset><legend>Shot history</legend>
    ...
    <small>Up to 120 shots. Time applies the configured timezone to uptime since boot. CSV includes boot id and calculated local time.</small>
  </fieldset>
...
<script>
'use strict';
let token=sessionStorage.getItem('shotStopperToken')||'',csrf=sessionStorage.getItem('shotStopperCsrf')||'',lastLog=0,configRevision=0,configLoaded=false,statusBusy=false,logBusy=false,shotsBusy=false,heartbeatBusy=false,scanBusy=false,scanTimer=0,controlsMutable=false,shotHistory={bootId:0,timezoneOffsetMinutes:0,shots:[]};
...
function formatShotTime(r,tz){let adjusted=Math.floor(r.endedAtMs/1000)+tz*60;const day=Math.floor(adjusted/86400);adjusted=((adjusted%86400)+86400)%86400;const time=pad2(Math.floor(adjusted/3600))+':'+pad2(Math.floor((adjusted%3600)/60))+':'+pad2(adjusted%60);return day>0?'#'+r.bootId+' d'+day+' '+time:'#'+r.bootId+' '+time}
function formatShotTimeCsv(r,tz){let adjusted=Math.floor(r.endedAtMs/1000)+tz*60;const day=Math.floor(adjusted/86400);adjusted=((adjusted%86400)+86400)%86400;const time=pad2(Math.floor(adjusted/3600))+':'+pad2(Math.floor((adjusted%3600)/60))+':'+pad2(adjusted%60);return day>0?'d'+day+' '+time:time}
...
function renderShots(){const body=$('shotRows');body.replaceChildren();const cols=10;if(!shotHistory.shots.length){const row=document.createElement('tr');row.innerHTML='<td colspan="'+cols+'">No recorded shots yet.</td>';body.appendChild(row);return}for(const r of shotHistory.shots){const row=document.createElement('tr');const err=r.errorPct===null||r.errorPct===undefined?'—':r.errorPct.toFixed(1)+'%';row.innerHTML='<td>'+formatShotTime(r,shotHistory.timezoneOffsetMinutes)+'</td><td>'+r.durationS.toFixed(1)+'s</td><td>'+r.goalG+'g</td><td>'+(r.actualG===null?'—':r.actualG.toFixed(1)+'g')+'</td><td>'+err+'</td><td>'+(r.avgFlowGS===null?'—':r.avgFlowGS.toFixed(2)+' g/s')+'</td><td>'+(r.firstDropS===null?'—':r.firstDropS.toFixed(1)+'s')+'</td><td>'+r.shotType+'</td><td>'+r.cutType+'</td>';const del=document.createElement('td');if(authenticated()){const btn=document.createElement('button');btn.textContent='Delete';btn.onclick=()=>deleteOneShot(r.id);del.appendChild(btn)}else del.textContent='—';row.appendChild(del);body.appendChild(row)}}
...
function exportShotsCsv(){const rows=[['id','boot_id','local_time','duration_s','goal_g','actual_g','error_g','error_pct','offset_g','avg_flow_g_s','first_drop_s','shot_type','cut_type','ended_at_ms','timezone_offset_min']];for(const r of shotHistory.shots)rows.push([r.id,r.bootId,formatShotTimeCsv(r,shotHistory.timezoneOffsetMinutes),r.durationS,r.goalG,r.actualG??'',r.errorG??'',r.errorPct??'',r.offsetG,r.avgFlowGS??'',r.firstDropS??'',r.shotType,r.cutType,r.endedAtMs,shotHistory.timezoneOffsetMinutes]);const csv=rows.map(c=>c.map(v=>{const s=String(v);return /[",\n]/.test(s)?'"'+s.replace(/"/g,'""')+'"':s}).join(',')).join('\n');const blob=new Blob([csv],{type:'text/csv'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='shot-history.csv';a.click();URL.revokeObjectURL(a.href)}
...
function loadConfig(c){if(configLoaded&&configRevision===c.revision)return;$('goalWeightG').value=c.goalWeightG;[['rinseGestureS','rinseGestureMs'],['rinseDurationS','rinseDurationMs'],['brewConfirmS','brewConfirmMs'],['minAutoStopS','minAutoStopMs'],['operationalWallS','operationalWallMs'],['paddleReturnReminderIntervalS','paddleReturnReminderIntervalMs'],['paddleReturnReminderMaxDurationS','paddleReturnReminderMaxDurationMs']].forEach(([ui,api])=>$(ui).value=String(c[api]/1000));populateTimezoneOptions();$('timezoneOffsetMinutes').value=String(c.timezoneOffsetMinutes??0);['autoTare','timerOnly','canTareStartTimer','brewConfirmationBeep','paddleReturnReminderBeep'].forEach(k=>$(k).checked=!!c[k]);configRevision=c.revision;configLoaded=true;if(typeof c.timezoneOffsetMinutes==='number')shotHistory.timezoneOffsetMinutes=c.timezoneOffsetMinutes}
...
$('logoutButton').onclick=async()=>{try{await api('/api/v1/logout',{method:'POST',body:'{}'})}catch(_){}logoutLocal()};$('virtualPaddle').onchange=()=>command('/api/v1/control/paddle',{on:$('virtualPaddle').checked});$('rinseButton').onclick=()=>command('/api/v1/control/rinse');$('stopButton').onclick=()=>command('/api/v1/control/stop');$('restartButton').onclick=()=>{if(confirm('Restart the controller?'))command('/api/v1/control/restart')};$('scanNetworkButton').onclick=startWifiScan;$('staNetwork').onchange=selectDetectedNetwork;$('staOpen').onchange=updateNetworkPasswordState;$('saveConfigButton').onclick=()=>command('/api/v1/config',{goalWeightG:number('goalWeightG'),rinseGestureMs:sToMs('rinseGestureS'),rinseDurationMs:sToMs('rinseDurationS'),brewConfirmMs:sToMs('brewConfirmS'),minAutoStopMs:sToMs('minAutoStopS'),operationalWallMs:sToMs('operationalWallS'),autoTare:$('autoTare').checked,timerOnly:$('timerOnly').checked,canTareStartTimer:$('canTareStartTimer').checked,brewConfirmationBeep:$('brewConfirmationBeep').checked,paddleReturnReminderBeep:$('paddleReturnReminderBeep').checked,paddleReturnReminderIntervalMs:sToMs('paddleReturnReminderIntervalS'),paddleReturnReminderMaxDurationMs:sToMs('paddleReturnReminderMaxDurationS'),timezoneOffsetMinutes:number('timezoneOffsetMinutes')});$('resetCalibrationButton').onclick=()=>{if(confirm('Reset the learned stop offset to the default 1.5 g? This cannot be undone.'))command('/api/v1/calibration/reset')};$('saveNetworkButton').onclick=()=>command('/api/v1/network',{action:'save',ssid:$('staSsid').value,password:$('staPassword').value,open:$('staOpen').checked}).finally(()=>{$('staPassword').value='' });$('forgetNetworkButton').onclick=()=>{if(confirm('Forget the STA network and restart?'))command('/api/v1/network',{action:'forget'})};$('changeApPasswordButton').onclick=()=>{const n=$('newApPassword').value;if(n!==$('confirmApPassword').value){message('The new passwords do not match.','error');return}command('/api/v1/access-point/password',{currentPassword:$('currentApPassword').value,newPassword:n}).finally(()=>{['currentApPassword','newApPassword','confirmApPassword'].forEach(id=>$(id).value='')})};$('factoryResetButton').onclick=()=>{if(confirm('Restore every stopper setting to factory defaults? This erases Wi-Fi credentials, workflow changes, learned calibration, and the custom AP/UI password, then restarts the controller. This cannot be undone.'))command('/api/v1/factory-reset',{confirm:'ERASE_ALL_SETTINGS'})};$('copyLogButton').onclick=()=>navigator.clipboard&&navigator.clipboard.writeText($('log').value);$('clearLogButton').onclick=()=>{$('log').value=''};$('exportShotsButton').onclick=exportShotsCsv;$('clearShotsButton').onclick=clearShotHistory;updateAccessMode();setMutable(false);populateTimezoneOptions();refreshStatus();refreshShots();refreshLog();if(authenticated())message('Session active.','ok');setInterval(()=>refreshStatus(),1000);setInterval(()=>refreshLog(),3000);setInterval(()=>refreshShots(),15000);setInterval(()=>{if(authenticated())heartbeat()},10000);
</script>
</body></html>)HTML";
