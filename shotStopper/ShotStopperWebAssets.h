...
<main id="app">
  ...
  <fieldset id="shotPanel"><legend>Shot</legend>
    <p id="shotIdle">No active shot.</p>
    ...
    <div class="statusColumn">
      ...
      <div class="metric"><strong>Extraction guard</strong><div id="shotGuard">—</div></div>
    </div>
  ...
  <fieldset id="actionsPanel" class="authenticatedOnly hidden"><legend>Actions</legend>
    ...
    <small id="controlPolicy">Paddle has priority. Stop opens CN9 only.</small>
  </fieldset>
  ...
  <fieldset id="workflowPanel"><legend>Configuration</legend>
    ...
    <details class="cfgGroup"><summary>Brew & weight</summary>
      <div class="row">
        <label>Target (g)<input id="goalWeightG" type="number" min="10" max="200" required><small class="fieldHint">Stop weight (near target − offset).</small></label>
        <label>CN9 limit (s)<input id="operationalWallS" type="number" min="5" max="60" step="1" required><small class="fieldHint">Max CN9 closed time (hard cap 60 s).</small></label>
      </div>
      <div class="metric"><strong>Learned stop offset (g)</strong><div id="learnedOffsetG">—</div></div>
      <label>Brew start confirmation (s)<input id="confirmationTimeoutS" type="number" min="0.5" max="30" step="0.1" required><small class="fieldHint">≥ retare + 3 s; blocks weight stop until done.</small></label>
    </details>
    <details class="cfgGroup"><summary>Fast extraction guard</summary>
      ...
      <label class="fastGuardOpt">Max recovery (g)<input id="maxRecoveryWeightG" type="number" min="10" max="200" step="0.1" required></label>
      <label class="fastGuardOpt">Min brew time (s)<input id="minBrewTimeS" type="number" min="5" max="55" step="0.5" required></label>
    </details>
    <details class="cfgGroup"><summary>A→M time guard</summary>
      ...
      <label class="atmManualOpt">Manual limit (s)<input id="autoToManualGuardManualLimitS" type="number" min="10" max="60" step="1" required></label>
    ...
    <details class="cfgGroup"><summary>Quick rinse</summary>
      <div class="row">
        <label>Rinse gesture (s)<input id="rinseGestureS" type="number" min="0.1" max="5" step="0.1" required><small class="fieldHint">Max paddle ON time that still counts as a rinse.</small></label>
        <label>Rinse duration (s)<input id="rinseDurationS" type="number" min="0.5" max="10" step="0.1" required><small class="fieldHint">CN9 closed duration after rinse starts.</small></label>
      </div>
    </details>
    <details class="cfgGroup"><summary>Scale & retare</summary>
      ...
      <label><input id="timerOnly" type="checkbox"> Timer only; do not stop by weight<small class="fieldHint">Tare/timer only; no weight stop.</small></label>
      <label><input id="canTareStartTimer" type="checkbox"> Bookoo combined command<small class="fieldHint">Bookoo tare+start; requires automatic tare.</small></label>
      <label><input id="autoRetare" type="checkbox"> Enable automatic retare<small class="fieldHint">Late cup: one tare during the retare window.</small></label>
      <label class="retareOpt">Retare window (s)<input id="retareWindowS" type="number" min="0.5" max="10" step="0.1" required><small class="fieldHint">Late-cup retare window.</small></label>
      <label class="retareOpt">Minimum cup weight (g)<input id="minimumCupWeightG" type="number" min="1" max="500" step="0.1" required><small class="fieldHint">Min load treated as a cup for retare.</small></label>
      <label class="retareOpt">Retare stable samples<input id="retareStabilitySamples" type="number" min="2" max="10" step="1" required><small class="fieldHint">Stable readings before retare.</small></label>
      <label class="retareOpt">Retare stability tolerance (g)<input id="retareStabilityToleranceG" type="number" min="0.1" max="20" step="0.1" required><small class="fieldHint">Max change between stable readings.</small></label>
      <label class="retareOpt">Retare sample gap (s)<input id="retareStabilityMaxGapS" type="number" min="0.1" max="5" step="0.1" required><small class="fieldHint">Max gap between readings.</small></label>
      <label class="retareOpt">Retare min stable time (s)<input id="retareStabilityMinDurationS" type="number" min="0" max="2" step="0.1" required><small class="fieldHint">Min stable time; 0 disables.</small></label>
    </details>
    <details class="cfgGroup"><summary>Alerts</summary>
      <label><input id="brewConfirmationBeep" type="checkbox"> Beep when coffee starts<small class="fieldHint">Beep on first coffee drop.</small></label>
      ...
      <label class="paddleOpt">Paddle reminder interval (s)<input id="paddleReturnReminderIntervalS" type="number" min="5" max="60" step="1" required><small class="fieldHint">Seconds between paddle-return beeps.</small></label>
      <label class="paddleOpt">Paddle reminder limit (min)<input id="paddleReturnReminderMaxDurationMin" type="number" min="1" max="60" step="1" required><small class="fieldHint">Stop beeps after this many minutes.</small></label>
    </details>
    ...
    <small>Required: rinse gesture &lt; CN9 limit ≤ 60 s; retare + confirmation ≤ CN9 limit.</small>
  </fieldset>
  ...
  <fieldset id="networkPanel" class="authenticatedOnly hidden"><legend>Wi-Fi</legend>
    ...
    <label><input id="staOpen" type="checkbox"> Open network<small class="fieldHint">Open network; leave password empty.</small></label>
  ...
  <fieldset id="shotHistoryPanel"><legend>Shot history</legend>
    <div id="shotTableWrap"><table id="shotTable"><thead><tr><th>Time</th><th>Dur</th><th>Goal</th><th>Actual</th><th>Err%</th><th class="colFlow">Flow</th><th>1st drop</th><th>Guard</th><th>Ext</th><th>Stop</th><th>Shot</th><th class="colCut">Cut</th><th></th></tr></thead><tbody id="shotRows"><tr><td colspan="13">Loading…</td></tr></tbody></table></div>
...
<script>
'use strict';
let token=sessionStorage.getItem('shotStopperToken')||'',csrf=sessionStorage.getItem('shotStopperCsrf')||'',lastLog=0,configRevision=0,configLoaded=false,configDirty=false,statusBusy=false,logBusy=false,shotsBusy=false,heartbeatBusy=false,scanBusy=false,scanTimer=0,controlsMutable=false,statusTimezoneOffsetMinutes=0,firmwareVersion='',shotHistory={bootId:0,shots:[]},logEvents=[];
...
function renderShots(){const body=$('shotRows');body.replaceChildren();const cols=13;if(!shotHistory.shots.length){const row=document.createElement('tr');row.innerHTML='<td colspan="'+cols+'">No recorded shots yet.</td>';body.appendChild(row);return}for(const r of shotHistory.shots){const row=document.createElement('tr');const err=r.errorPct===null||r.errorPct===undefined?'—':r.errorPct.toFixed(1)+'%';const guard=r.extractionGuardEnabled?'on':'off';const ext=r.extractionExtended?'yes':'no';const stop=r.stopDetail||'—';row.innerHTML='<td>'+formatShotTime(r)+'</td><td>'+r.durationS.toFixed(1)+'s</td><td>'+r.goalG+'g</td><td>'+(r.actualG===null?'—':r.actualG.toFixed(1)+'g')+'</td><td>'+err+'</td><td class="colFlow">'+(r.avgFlowGS===null?'—':r.avgFlowGS.toFixed(2)+' g/s')+'</td><td>'+(r.firstDropS===null?'—':r.firstDropS.toFixed(1)+'s')+'</td><td>'+guard+'</td><td>'+ext+'</td><td>'+stop+'</td><td>'+r.shotType+'</td><td class="colCut">'+r.cutType+'</td>';const del=document.createElement('td');if(authenticated()){const btn=document.createElement('button');btn.textContent='Delete';btn.onclick=()=>deleteOneShot(r.id);del.appendChild(btn)}else del.textContent='—';row.appendChild(del);body.appendChild(row)}}
function updateFirmwareFooter(){const fw=firmwareVersion||'—';const boot=shotHistory.bootId?('#'+shotHistory.bootId):'—';$('firmwareFooter').textContent='Firmware '+fw+' · Boot '+boot}
...
function updateConfigGroups(){document.querySelectorAll('.retareOpt').forEach(e=>e.classList.toggle('fieldOff',!$('autoRetare').checked));document.querySelectorAll('.paddleOpt').forEach(e=>e.classList.toggle('fieldOff',!$('paddleReturnReminderBeep').checked));document.querySelectorAll('.fastGuardOpt').forEach(e=>e.classList.toggle('fieldOff',!$('fastExtractionGuardEnabled').checked));document.querySelectorAll('.atmManualOpt').forEach(e=>e.classList.toggle('fieldOff',$('autoToManualGuardLimitMode').value!=='manual'))}
function validateConfigClient(){const rg=number('rinseGestureS'),rd=number('rinseDurationS'),wall=number('operationalWallS'),rw=number('retareWindowS'),conf=number('confirmationTimeoutS');if(!(rg<wall))return'Rinse gesture must be less than the CN9 limit.';if(rd>wall)return'Rinse duration must not exceed the CN9 limit.';if(rw+conf>wall)return'Retare window + brew start confirmation must not exceed the CN9 limit.';const minConf=($('autoRetare').checked?rw:0)+3;if(conf<minConf)return'Brew start confirmation must be at least retare window + 3 s.';if($('canTareStartTimer').checked&&!$('autoTare').checked)return'The Bookoo combined command requires automatic tare.';if($('paddleReturnReminderBeep').checked&&number('paddleReturnReminderMaxDurationMin')*60<number('paddleReturnReminderIntervalS'))return'Paddle limit must be at least the reminder interval.';if($('fastExtractionGuardEnabled').checked){const goal=number('goalWeightG'),maxW=number('maxRecoveryWeightG'),minT=number('minBrewTimeS');if(!(maxW>goal))return'Max recovery must exceed target.';if(minT>=wall)return'Min brew time must be below CN9 limit.';if(minT<conf)return'Min brew time must be at least brew confirmation.'}const atmLimit=number('autoToManualGuardManualLimitS');if(atmLimit<10||atmLimit>wall)return'A→M manual limit must be 10 s…CN9 limit.';return''}
function configPayload(){return{goalWeightG:number('goalWeightG'),rinseGestureMs:sToMs('rinseGestureS'),rinseDurationMs:sToMs('rinseDurationS'),operationalWallMs:sToMs('operationalWallS'),retareWindowMs:sToMs('retareWindowS'),confirmationTimeoutMs:sToMs('confirmationTimeoutS'),minimumCupWeightG:number('minimumCupWeightG'),retareStabilitySamples:number('retareStabilitySamples'),retareStabilityToleranceG:number('retareStabilityToleranceG'),retareStabilityMaxGapMs:sToMs('retareStabilityMaxGapS'),retareStabilityMinDurationMs:sToMs('retareStabilityMinDurationS'),autoTare:$('autoTare').checked,timerOnly:$('timerOnly').checked,canTareStartTimer:$('canTareStartTimer').checked,brewConfirmationBeep:$('brewConfirmationBeep').checked,paddleReturnReminderBeep:$('paddleReturnReminderBeep').checked,autoRetare:$('autoRetare').checked,paddleReturnReminderIntervalMs:sToMs('paddleReturnReminderIntervalS'),paddleReturnReminderMaxDurationMs:Math.round(number('paddleReturnReminderMaxDurationMin')*60000),fastExtractionGuardEnabled:$('fastExtractionGuardEnabled').checked,maxRecoveryWeightG:number('maxRecoveryWeightG'),minBrewTimeMs:sToMs('minBrewTimeS'),autoToManualGuardEnabled:$('autoToManualGuardEnabled').checked,autoToManualGuardLimitMode:$('autoToManualGuardLimitMode').value,autoToManualGuardManualLimitMs:sToMs('autoToManualGuardManualLimitS'),timezoneOffsetMinutes:number('timezoneOffsetMinutes'),ntpServerPreset:$('ntpServerPreset').value,ntpServerCustom:$('ntpServerCustom').value.trim()}}
