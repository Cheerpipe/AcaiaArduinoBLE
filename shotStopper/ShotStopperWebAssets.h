...
<main id="app">
  ...
  <fieldset id="workflowPanel"><legend>Workflow</legend>
    ...
    <label>Paddle reminder interval (ms)<input id="paddleReturnReminderIntervalMs" type="number" min="5000" max="60000" required></label>
    <label>Paddle reminder limit (ms)<input id="paddleReturnReminderMaxDurationMs" type="number" min="60000" max="3600000" required></label>
...
function loadConfig(c){if(configLoaded&&configRevision===c.revision)return;['goalWeightG','rinseGestureMs','rinseDurationMs','brewConfirmMs','minAutoStopMs','operationalWallMs','paddleReturnReminderIntervalMs','paddleReturnReminderMaxDurationMs'].forEach(k=>$(k).value=c[k]);['autoTare','timerOnly','canTareStartTimer','brewConfirmationBeep','paddleReturnReminderBeep'].forEach(k=>$(k).checked=!!c[k]);configRevision=c.revision;configLoaded=true}
...
$('logoutButton').onclick=async()=>{try{await api('/api/v1/logout',{method:'POST',body:'{}'})}catch(_){}logoutLocal()};$('virtualPaddle').onchange=()=>command('/api/v1/control/paddle',{on:$('virtualPaddle').checked});$('rinseButton').onclick=()=>command('/api/v1/control/rinse');$('stopButton').onclick=()=>command('/api/v1/control/stop');$('restartButton').onclick=()=>{if(confirm('Restart the controller?'))command('/api/v1/control/restart')};$('scanNetworkButton').onclick=startWifiScan;$('staNetwork').onchange=selectDetectedNetwork;$('staOpen').onchange=updateNetworkPasswordState;$('saveConfigButton').onclick=()=>command('/api/v1/config',{goalWeightG:number('goalWeightG'),rinseGestureMs:number('rinseGestureMs'),rinseDurationMs:number('rinseDurationMs'),brewConfirmMs:number('brewConfirmMs'),minAutoStopMs:number('minAutoStopMs'),operationalWallMs:number('operationalWallMs'),autoTare:$('autoTare').checked,timerOnly:$('timerOnly').checked,canTareStartTimer:$('canTareStartTimer').checked,brewConfirmationBeep:$('brewConfirmationBeep').checked,paddleReturnReminderBeep:$('paddleReturnReminderBeep').checked,paddleReturnReminderIntervalMs:number('paddleReturnReminderIntervalMs'),paddleReturnReminderMaxDurationMs:number('paddleReturnReminderMaxDurationMs')});$('resetCalibrationButton').onclick=()=>{if(confirm('Reset the learned stop offset to the default 1.5 g? This cannot be undone.'))command('/api/v1/calibration/reset')};$('saveNetworkButton').onclick=()=>command('/api/v1/network',{action:'save',ssid:$('staSsid').value,password:$('staPassword').value,open:$('staOpen').checked}).finally(()=>{$('staPassword').value='' });$('forgetNetworkButton').onclick=()=>{if(confirm('Forget the STA network and restart?'))command('/api/v1/network',{action:'forget'})};$('changeApPasswordButton').onclick=()=>{const n=$('newApPassword').value;if(n!==$('confirmApPassword').value){message('The new passwords do not match.','error');return}command('/api/v1/access-point/password',{currentPassword:$('currentApPassword').value,newPassword:n}).finally(()=>{['currentApPassword','newApPassword','confirmApPassword'].forEach(id=>$(id).value='')})};$('factoryResetButton').onclick=()=>{if(confirm('Restore every stopper setting to factory defaults? This erases Wi-Fi credentials, workflow changes, learned calibration, and the custom AP/UI password, then restarts the controller. This cannot be undone.'))command('/api/v1/factory-reset',{confirm:'ERASE_ALL_SETTINGS'})};$('copyLogButton').onclick=()=>navigator.clipboard&&navigator.clipboard.writeText($('log').value);$('clearLogButton').onclick=()=>{$('log').value=''};updateAccessMode();setMutable(false);refreshStatus();refreshLog();if(authenticated())message('Session active.','ok');setInterval(()=>refreshStatus(),1000);setInterval(()=>refreshLog(),3000);setInterval(()=>{if(authenticated())heartbeat()},10000);
</script>
</body></html>)HTML";
