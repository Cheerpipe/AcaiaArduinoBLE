...
<main id="app">
  ...
  <fieldset id="workflowPanel"><legend>Configuration</legend>
    ...
    <details class="cfgGroup"><summary>Scale &amp; retare</summary>
      ...
      <label class="retareOpt">Retare window (s)<input id="retareWindowS" type="number" min="0.5" max="10" step="0.1" required><small class="fieldHint">Time after shot start to detect a late cup and retare; ignored when automatic retare is off.</small></label>
      <label class="retareOpt">Minimum cup weight (g)<input id="minimumCupWeightG" type="number" min="1" max="500" step="0.1" required><small class="fieldHint">Minimum stable load treated as a cup for retare only; lighter loads are ignored for retare detection.</small></label>
      <label class="retareOpt">Retare stable samples<input id="retareStabilitySamples" type="number" min="2" max="10" step="1" required><small class="fieldHint">Consecutive stable readings required before retare; higher values reduce false retares.</small></label>
      <label class="retareOpt">Retare stability tolerance (g)<input id="retareStabilityToleranceG" type="number" min="0.1" max="20" step="0.1" required><small class="fieldHint">Maximum weight change allowed between stable readings; lower values require a steadier cup.</small></label>
      <label class="retareOpt">Retare sample gap (s)<input id="retareStabilityMaxGapS" type="number" min="0.1" max="5" step="0.1" required><small class="fieldHint">Maximum time between stable readings; longer gaps restart the stability counter.</small></label>
      <label class="retareOpt">Retare min stable time (s)<input id="retareStabilityMinDurationS" type="number" min="0" max="2" step="0.1" required><small class="fieldHint">Minimum time a stable load must persist before retare; blocks retare on very fast reading bursts. Use 0 to disable.</small></label>
    </details>
    <details class="cfgGroup"><summary>Alerts &amp; reminders</summary>
      ...
      <label class="paddleOpt">Paddle reminder limit (s)<input id="paddleReturnReminderMaxDurationS" type="number" min="60" max="3600" step="1" required><small class="fieldHint">Stop reminder beeps after this duration even if the paddle remains ON.</small></label>
    </details>
  ...
  <fieldset class="authenticatedOnly hidden"><legend>Factory reset</legend>
    ...
    <small>This removes the STA network, configuration changes, learned calibration, shot history, and the custom AP/UI password, then restarts the controller.</small>
  </fieldset>
...
function updH(h){function b(n){return typeof n!=='number'||n<0?'—':n>=1048576?(n/1048576).toFixed(1)+'M':n>=1024?Math.round(n/1024)+'K':n+'b'}const s=(i,v)=>$(i).textContent=v||'—';if(!h){['hCpu','hTemp','hTPeak','hRamT','hRamU','hRamF'].forEach(i=>s(i,'—'));return}s('hCpu',typeof h.cpuUsagePct==='number'?h.cpuUsagePct+'%':'');s('hTemp',h.tempValid?h.tempC.toFixed(1)+'°C':'');s('hTPeak',h.tempValid?h.tempPeakC.toFixed(1)+'°C':'');s('hRamT',b(h.ramTotalBytes));s('hRamU',b(h.ramUsedBytes));s('hRamF',b(h.ramFreeBytes))}
