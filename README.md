...
| **Avoid BBW shot without scale** | **On by default**. Machine-level (Settings → Brew, subgroup below Brew by Weight; not per-preset). With Brew by weight on and no usable scale, a paddle ON longer than the **quick rinse gesture** plays the existing no-scale triple beep and **does not close CN9**. A shorter ON→OFF is a rinse: CN9 closes and the guard stays **Armed**. The next start after a blocked shot runs as a manual no-scale shot. Re-arms on boot, when the scale becomes available, or after **Last shot cooldown**. |
