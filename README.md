...
| **Automatic retare** | Allow one late-cup retare during the retare window (default ON). Fires on the cup-presence **placed** event. |
| **Retare window (s)** | Time after shot start to detect and retare a late-placed cup (default 4 s). |
| **Minimum cup weight (g)** | Machine-level (**Settings → Machine and scale → Cup**). Stable load that counts as a cup being placed; positive hysteresis bound (default 10 g). |
| **Cup-removed threshold (g)** | Machine-level (**Cup**). Confirmed weight at or below this means the cup was lifted (default −3 g). |
| **Cup placement stability** | Samples (default 3), tolerance (default 2.0 g), max sample gap (default 0.5 s), and min stable time (default 0.3 s) required before cup-present. |
...
The **official supported** firmware build is **ESP-IDF** via `./scripts/build-idf`
(Arduino-ESP32 as a component). You also need a C++ compiler for host tests and
Node.js to validate Web UI assets. Optional coverage requires LLVM
(`llvm-profdata` and `llvm-cov`).

### ESP-IDF (required)

Install ESP-IDF **5.5.x** (this project tracks **5.5.5**, matching Arduino-ESP32
3.3.11). Example on macOS/Linux:
...
active at 20/20 ms (100% duty), and Shot Stopper keeps that default so
SCAN_RSP names stay visible. The script reverts leftover 40/20 or 100/30 GAP
params, makes discovery OOM-safe (placement `new`), and places ArduinoBLE
**host** objects (GAP queue, ATT remote tree, local GATT values) in PSRAM via
`BLEHostAlloc` while VHCI stream buffers and the BLE **controller** stay on
internal SRAM. `./scripts/build-idf` (and the legacy `./scripts/build`) run
the patch automatically. The script is idempotent. Set `ARDUINO_BLE_HOME` if
ArduinoBLE is not in the default path.

Shot Stopper requires ESP32-S3 with PSRAM so Web UI buffers can live in
SPIRAM while the BLE controller stays on internal SRAM. The IDF project enables
`CONFIG_SPIRAM_ALLOW_BSS_SEG_EXTERNAL_MEMORY` so large BSS can land in PSRAM.
After a core/IDF bump, run the BLE+Web UI hardware gate in
[Manual Test Plan](docs/MANUAL_TEST_PLAN.md) (M72 smoke, M73 short soak).
...
The radio stays in scan; the controller runs the stock 20/20 cycle. The only
...
audited version is included in `libraries/AcaiaArduinoBLE`.

### Optional static analysis (recommended)

[`Cppcheck`](https://cppcheck.sourceforge.io/) is the recommended optional tool
for static analysis of the C++ firmware. Prefer the IDF compilation database:
...
This recompiles with `-fanalyzer` into `reports/gcc-analyzer/`.

### Legacy arduino-cli (unsupported)

`./scripts/build` and related arduino-cli helpers remain in the tree for
transition only. They are **not** the supported production path: prebuilt
Arduino-ESP32 libraries compile `ALLOW_BSS` out, so large BSS cannot move to
PSRAM the way the IDF project does. If you still use them temporarily:
...
On Arduino-ESP32 **3.3.6+** the core frees BLE controller RAM at boot unless a
translation unit includes `esp32-hal-alloc-ble-mem.h`. The vendored
...
```sh
strings build-idf/n16r8/shotstopper.bin | grep -E '^[0-9]+\\.[0-9]+\\.[0-9]+\\+'
```
...
(`build-idf` already runs `gen_version.sh` and `gen_web_ui.js`.) Extra compile
defines go in `--flags`, for example remote CN9 and buzzer:

```sh
./scripts/build-idf --arch n16r8 \
  --flags "-Werror=deprecated-copy -DSHOT_STOPPER_ENABLE_REMOTE_CN9=1 -DSHOT_STOPPER_ENABLE_BUZZER=1"
```

For N8R4: `./scripts/build-idf --arch n8r4`.
...
N16R8 uses the normal 16 MB OTA scheme (`partitions-n16r8.csv`, **3 MB** app
slot × 2). N8R4 uses the 8 MB table (**~3.2 MB** × 2). The 4 MB `default` table
only allows **1.25 MB** and cannot hold this firmware.
...
Connect the board over USB and open the official IDF monitor:
...
rate after the app starts). Exit the IDF monitor with **Ctrl+]**. Commands are
listed in [USB serial CLI](docs/SERIAL_CLI.md). Debug paddle/CN9/Wi-Fi traces
are **off** by default; enable them from Debug (**Serial debug output**) or
`SERIAL_DEBUG_ON`.

You should see CLI replies immediately. With serial debug on, lines such as
`Shot Stopper Micra …` and `Goal weight: …` also appear.
Press **RST** on the board if the monitor was already open.
...
- Confirm the port (for example with `ls /dev/cu.usbmodem*` on macOS) and use
  the `cu.*` device on macOS.
