...
| **A veces mi shot termina antes o después del peso objetivo y la balanza no se detiene el flujo.** | Puede deberse al **Auto-to-manual time guard** (*A→M time guard*), activado **por defecto**. Si pierdes la balanza durante un shot automático (BLE caído o stream obsoleto), el firmware reintenta reconectar unos segundos y luego continúa en **manual** sin parar por peso. El guard cierra CN9 en un plazo más corto y predecible (p. ej. **30 s** en modo manual, o una estimación por tendencia en modo auto) para evitar un shot larguísimo. Revisa el panel en vivo: aparece `A→M · …s` cuando está activo. |
...
| **La balanza se desconectó a mitad del shot. ¿Qué pasa?** | En shots automáticos el firmware abre una **ventana de reconexión** (por defecto **3 s**) con reintentos BLE agresivos. Si recupera tres muestras coherentes a tiempo, vuelve el control por peso y el **A→M time guard** se desactiva. Si no, el resto del ciclo pasa a **manual** (sin stop por peso aunque la balanza vuelva). En ese momento también entra en juego el **Auto-to-manual time guard** (activo por defecto): fija un **plazo máximo desde el inicio del ciclo** (p. ej. **30 s** en modo manual, o una estimación por tendencia en modo auto) y puede **cerrar CN9 antes** del límite CN9 configurado — incluso mientras aún se reintenta la reconexión BLE. En el panel en vivo verás `A→M · …s`. El paddle OFF y el tope duro de **60 s** siguen siendo válidos. |
...
| Parada temprana/tardía por pérdida de balanza | **A→M time guard** (Enable, Limit mode, Manual limit) |
...
| Reconexión BLE en brew | **Max scale disconnect during brew (s)** |
