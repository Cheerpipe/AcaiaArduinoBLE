...
| **El shot llega al peso objetivo pero no se detiene.** | Durante los primeros segundos aplica la **protección BBW / pre-arm** (*BBW protection*): ventana contra paradas accidentales (toque del vaso, tara lenta, ruido de peso). **Por defecto 12 s.** Hasta que termina esa ventana (o se detectan las primeras gotas), el stop automático por peso está inhibido. También puede bloquearlo la **ventana de retarificación** si aún está activa. Corre en paralelo al retare desde el paddle ON. |
...
| **¿Cómo accedo cuando ya guardé mi Wi‑Fi de casa?** | El dispositivo usa modo **STA**; abre **`http://&lt;ip-del-dispositivo&gt;`**. La IP aparece en los logs serie (**9600** baud), en la lista DHCP del router, o es la IP fija que configuraste. La contraseña Web es la que configuraste (o **`Micra1234`** si no la cambiaste). Si una IP fija nueva no es alcanzable, en ~3 min se restaura la config anterior y vuelve el SoftAP **`MicraShotStopperAP`**. |
