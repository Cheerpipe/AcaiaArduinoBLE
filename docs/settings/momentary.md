...
| **Shot reaction timeout (s)** | 12 | 3–30; `0` in JSON is the compiled 12 s | Switch-only. How long a quiet pan after Start may stay Assumed on before becoming Assumed off. Does not pulse the relay. Late espresso-like flow from Assumed off still confirms ON. |
