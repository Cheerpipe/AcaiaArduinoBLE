...
rinse gesture** does not close the machine circuit. On paddle, a shorter
ON→OFF is still a rinse: machine circuit closes and the guard goes
...
| --- | --- | --- | --- |
| **Avoid BBW shot without scale** | ON | ON / OFF | Armed: a long activator ON does not close the machine circuit. On paddle, a rinse gesture still rinses and consumes Armed. On momentary, that hold stays open and then consumes Armed. |
