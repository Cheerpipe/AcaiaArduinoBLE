...
| M57 | On home, open DevTools Network: stay on `/` for ~10 s, then open `/log`, `/history`, `/presets`, and `/settings`. | Home only polls `/api/v1/status` (and heartbeat if signed in). Log polls `/api/v1/log` only while on `/log`. History polls `/api/v1/shots` only while on `/history`. Presets/Settings poll status only while on those views. Deep-link refresh of `/log` serves the SPA and shows the log view. |
