...
| M56 | Open the Web UI home, then reload and sign in while a scale is connected. | First HTML load is gzip (`Content-Encoding: gzip`). CSS loads from `/app.css?v=…` (gzip). Reload and post-login reload return **304** for HTML when the firmware is unchanged; CSS is cached via versioned immutable URL. Scale stays connected; no `PACKET_TIMEOUT`. Safari does not show “Device unreachable”. |
